...
def plot_top_anomalies(
    ...
    """
    Plot the top N anomalous contracts.
...
def plot_agency_anomaly_counts(
    df: pd.DataFrame,
    is_anomaly_column: str = "is_anomaly",
    agency_column: str = "agency",
) -> None:
    """
    Plot the count of anomalous contracts by agency.
    ...
    agency_counts["anomaly_percentage"] = (
        agency_counts["anomaly_count"] / agency_counts["total_count"] * 100
    )
    ...
    ax = sns.barplot(
        x="anomaly_percentage", y=agency_column, data=top_agencies, color="skyblue"
    )
...
def plot_value_vs_anomaly_score(
    ...
    scatter = plt.scatter(
        df[score_column],
        df[value_column],
        c=df[is_anomaly_column].astype(int),
        cmap="coolwarm",
        alpha=0.6,
    )
    ...
    # Add legend
    legend1 = plt.legend(
        *scatter.legend_elements(), title="Anomaly", labels=["Normal", "Anomalous"]
    )
